{
	...
	MemoQ.prototype.getTip = function (row, callback) {
		headers = {
			'accept': '*/*',
			'content-type': 'application/json; charset=UTF-8',
			// 'x-xsrf-token':document.cookie.match(/X-XSRF-TOKEN=(.+);?/)[1]
	...
	MemoQ.prototype.getTips = function(callback) {
		if(typeof callback!=='function') return console.log('没有回调函数');
		let rs = [], countdown = MemoQ.rows.length
		MemoQ.rows.forEach((row)=>{
			this.getTip(row, (data)=>{
...
<textarea id="ao-hiddenclip"></textarea>
	...
	mq.getRows((rows) => {
		...
		mq.getTips((e)=>{
			...
			// showTips
			log(stringifyTips())
			c.find('button[name="copyTips"]').css({background:'#0f0'})
		});
	});

	function stringifyTips(){
		return mq.tips.filter(e => e.type < 2).map(e => `${e.source}\t${e.target}`).join('\n')
	}
