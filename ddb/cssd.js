(function (g, undefined) {
	...
	function getVariable(name, context = ':root') {
		if(name!==undefined) {
			name = `--${name}`;
			if (document.readyState === COMPLETE) {
				return getObject(context)[name];
			} else {
				return Main.promise.then(function (v) {
					return getObject(context)[name];
				})
	...
	function setVariable(name, value, context = ':root') {
		if(name!==undefined && value!==undefined) {
			name = `--${name}`;
			Main.promise.then(function () {
				setMap(context, { [name]: value });
			})
		}
		return Main;
	}
	...
	Object.defineProperties(Main, {
		...
		getVar: { value: getVariable },
		setVar: { value: setVariable },
	});
